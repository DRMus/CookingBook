...
export class AuthService {
  ...
  async registration(userDto: CreateUser) {
    ...
    const candidate = await this.usersService.getOneUserByUsername(userDto.username);
    if (candidate) {
      throw new HttpException("Данный логин уже используется", HttpStatus.BAD_REQUEST);
    }
